...
const currentDialogData = computed<CurrentDialogProps[]>(() => [
  {
    id: 0,
    value: '请分析一下巴菲特的投资理念',
...
function getClassName(imgPlacement: CurrentDialogProps['imgPlacement']) {
  if (imgPlacement === 'left') {
    return 'p-2';
  } else if (imgPlacement === 'right') {
    return 'flex-y-center p-2 justify-end';
  }
}

function viewContentHtml(content: string){
...
<template>
  <NCard :bordered="false" size="small" class="card-wrapper">
    <div :class="getClassName(item.imgPlacement)" v-for="item in currentDialogData" :key="item.id">
      <div class="pr-12px p-2">
        {{ item.value }}
      </div>
      <div class="p-2" v-for="content in item.content">
        <div style="white-space: pre-line;" v-html="viewContentHtml(content.value)">
        </div>
        <!-- <h3 class="text-18px font-semibold">
          {{ content.title }}
        </h3>
        <p class="text-#999 leading-30px" style="white-space: pre-line;">{{ content.value }}</p> -->
      </div>

      <div v-if="item.imgPlacement === 'right'" class="size-40px shrink-0 overflow-hidden rd-1/2">
